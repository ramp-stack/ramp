...
use prism::event::{MouseEvent, MouseState, MouseButton, KeyboardEvent, KeyboardState};
...
impl EventHandler {
    fn on_input(&mut self, ctx: &mut Context, input: Input) {
        if let Some(event) = match input {
            ...
            Input::CameraFrame(image) => Some(Box::new(event::CameraFrame(image)) as Box<dyn prism::event::Event>),
            Input::PickedPhoto(image) => Some(Box::new(event::PickedPhoto(image)) as Box<dyn prism::event::Event>),
                    ...
                    TouchPhase::Moved => {
                        self.scroll.and_then(|(prev_x, prev_y)| {
                            ...
                            (scroll_x.abs() > 0.01 || scroll_y.abs() > 0.01)
                                .then_some(MouseState::Scroll(scroll_x, scroll_y))
                        })
                    }
                }.map(|state| Box::new(MouseEvent {
            ...
            Input::Mouse { state, button, .. } => {
                ...
                // emit the full button event first
                self.context.1.push(Box::new(event::MouseButtonEvent {
                    position: Some(self.mouse),
                    state:    ms,
                    button:   btn,
                }) as Box<dyn prism::event::Event>);
                // then emit MouseEvent with button populated
                Some(Box::new(MouseEvent {
                    position: Some(self.mouse),
                    state:    ms,
                    button:   Some(btn),
                }) as Box<dyn prism::event::Event>)
                    ...
                    key: match event.logical_key {
                        Key::Named(named) => Some(prism::event::Key::Named(match named {
                            ...
                            NamedKey::Super | NamedKey::Hyper | NamedKey::Meta
                                                 => Some(prism::event::NamedKey::Meta),
